import { type Arguments, commandArguments, numberOption } from "./arguments.ts"
import { originAuthorizer } from "@cua/policy"
/**
 ...
 * from a platform package this workspace does not depend on.
 */
...
export const runReplayCommand = (
  ...
  Effect.gen(function* () {
    ...
    const control = sessionControl({
      ...
      announce: (intervention, operatorUrl) =>
        Console.error(
          [
            "",
            `PAUSED at step ${intervention.stepId}: ${scrubberFor(inputs.success)(intervention.reason)}`,
            ...
            argv.switches.has("headed")
              ? `  the window to work in belongs to the application ${
                  BROWSER_APPLICATION
                }; see its live screen in the operator interface`
              : `  this run is headless, so there is no window to work in: see the operator interface for context. Re-run with --headed to act on it`,
            ""
          ].join("\n")
        )
    ...
    const services = Layer.mergeAll(
      playwrightSurface({ headless: !argv.switches.has("headed"), authorizeOrigin: originAuthorizer(policy) }),
      policyFrom(policy),
...
const program = Effect.gen(function* () {
  const argv = commandArguments({ switches: ['headed', 'json', 'handoff', 'assist', 'noAmend', 'help'], options: ['baseUrl', 'tenant', 'version', 'policy', 'operatorPort', 'handoffWait', 'expireSessionAfter', 'amendTo'], maxPositionals: 1, allowInputOptions: true })
  if (argv.switches.has("help") || argv.positionals[0] === undefined) {
    yield* Console.log(usage())
    return
  }

  numberOption(argv, "operatorPort", { maximum: 65535, integer: true })
  numberOption(argv, "handoffWait", { minimum: 0.001 })
  numberOption(argv, "expireSessionAfter", { minimum: 1, integer: true })
  ...
  if (Result.isFailure(artifact)) {
    yield* Console.error(`cannot run ${argv.positionals[0]}: ${artifact.failure.message}`)
    process.exitCode = 2
    return
  }
  ...
  if (unsafe.length > 0) {
    yield* Console.error(
      [`cannot run ${argv.positionals[0]}:`, ...unsafe.map(describeUnsafeRepeat)].join("\n  ")
    )
    process.exitCode = 2
    return
  }
  ...
  const override =
    tenant === undefined
      ...
      : loadOverride(OVERRIDES_DIRECTORY, tenant, argv.positionals[0])
  ...
  const applied: AppliedOverride | undefined =
    override.success === undefined
      ...
      : {
          ...
          source: `${OVERRIDES_DIRECTORY}/${override.success.tenant}/${argv.positionals[0]}.yaml`,
...
if (import.meta.main) Effect.runPromise(Effect.scoped(program)).catch(() => {
  console.error("Replay could not complete. Check the policy and evidence directory.")
  process.exitCode = 1
})
