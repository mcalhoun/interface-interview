...
/** Where the expiry toggle has to fire to land inside `open-savings-account`. */
...
it.live("waits out the slow panel instead of acting on it", () =>
  Effect.gen(function* () {
    ...
    expect(at(outcome, "recovery.detected", "open-savings-account")).toEqual([])
    expect(at(outcome, "checkpoint", "open-savings-account").map((event) => event.verdict)).toEqual([
      "held"
    ])
    ...
    const opened = at(outcome, "action", "open-savings-account")[0]!
...
it.live("re-authenticates a session that expired mid-flow and finishes the run", () =>
  Effect.gen(function* () {
    ...
    const stranded = outcome.result.steps.find((step) => step.id === "open-savings-account")
...
it.live("resumes at the step it was on rather than starting the flow again", () =>
  Effect.gen(function* () {
    ...
    expect(attempted[2]).toContain("/member?memberNumber=12345")
...
it.live("puts a recovery's own actions through the same policy chokepoint as a step's", () =>
  Effect.gen(function* () {
    ...
    const recoveryActions = at(outcome, "action", "open-savings-account")
    expect(recoveryActions.map((event) => event.action)).toEqual([
      "click", // the step's own action, which landed on Sign On
      "fill", // the password
      "click", // Sign On
      "navigate", // back to where the step began
      "click" // the step's action again
    ])
  })
)
...
it.live("stops the run when a condition will not clear within its declared bound", () =>
  Effect.gen(function* () {
    ...
    expect(failure.stepId).toBe("open-savings-account")
...
it.live("reports an unrecognised state as an ordinary checkpoint failure, not a recovery", () =>
  Effect.gen(function* () {
    // 1.0.0 declares no recoverable conditions. The same member, the same
    // application, the same interstitial — and nothing recovers, because
    // recovery is something an artifact declares rather than something the
    // engine knows. This is what keeps the rules reviewable and a tenant's
    // overrides possible.
...
const neverClears: CheckpointOutcome = {
  held: false,
  state: nowhere,
  waitedMillis: 0,
...
it.live("spends the run's shared recovery budget, and stops when it is gone", () =>
  Effect.gen(function* () {
    ...
    if (spent.outcome.attempted) {
      ...
      expect(spent.outcome.outcome.held).toBe(false)
    }
