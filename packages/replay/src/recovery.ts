...
/**
 * Everything a port operation can stop for.
 *
 * Wider than `SurfaceUnavailable`, because a remedy's Actions and a re-check's
 * reads pass the same Session guard and Policy gate a Step's do: a refusal is a
 * `ReplayFailure` in the result contract's own words rather than a broken
 * browser. Recovery is not a privileged path, so it inherits the same refusals.
 *
 * None of these is "the recovery failed". A condition that will not clear is a
 * *value* — see `RecoveryOutcome` — and the engine turns it into a
 * `recovery_exhausted` Hard Failure with everything a reader needs.
 */
export type RecoveryBlocked = SurfaceUnavailable | ReplayFailure | EvidenceUnwritable
...
export interface RecoveryPort {
  ...
  readonly recheck: Effect.Effect<CheckpointOutcome, RecoveryBlocked>
  /** Whether a rule's `detect` assertions hold of the screen right now. */
  readonly detected: (
    assertions: ReadonlyArray<Assertion>
  ) => Effect.Effect<boolean, RecoveryBlocked>
...
export interface RecoveryRequest {
   ...
   *
   * Only a `failed` verdict, and that is the point. `evaluate` tries the intended
   * state first and every declared Business Outcome branch second, so a screen the
   * Artifact already declares as a domain answer can never arrive here to be
   * retried as though it were a transient glitch. The narrow type is the
   * enforcement; see the ladder documented in `checkpoint.ts`.
   */
  readonly failed: Extract<CheckpointOutcome, { verdict: "failed" }>
...
): Effect.Effect<RecoveryOutcome, RecoveryBlocked> =>
  Effect.gen(function* () {
    ...
    // Stop as soon as the Checkpoint stops saying "failed". That includes a
    // declared Business Outcome surfacing part-way through a recovery: the
    // application has answered, and retrying an answer is the mistake the whole
    // ladder is ordered to prevent.
    const outcome = yield* Effect.repeat(attempt, {
      until: (result: CheckpointOutcome) => cleared(result),
...
const firstMatching = (
  ...
  port: RecoveryPort
): Effect.Effect<RecoverableCondition | undefined, RecoveryBlocked> =>
  Effect.gen(function* () {
...
/**
 * Whether the Checkpoint has stopped failing.
 *
 * `held` and `outcome` both count. One says the intended state arrived; the other
 * says the application gave a declared answer instead, which is a result rather
 * than something to keep retrying.
 */
const cleared = (outcome: CheckpointOutcome): boolean => outcome.verdict !== "failed"

const describeOutcome = (outcome: CheckpointOutcome): string => {
  switch (outcome.verdict) {
    case "held":
      return "the intended state was reached"
    case "outcome":
      return `the declared outcome ${outcome.code} was reached instead: ${outcome.because}`
    case "failed":
